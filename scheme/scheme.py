...
def apply_primitive(procedure, args_scheme_list, env):
    ...
    except TypeError as err: # If a different number of parameters are passed to the function, an error will be raised
...
class Promise(object):
    ...
    def evaluate(self):
        if self.forced:
            return self.result
        try:
            self.result = scheme_eval(self.expr.first, self.env.make_child_frame(nil, nil))
            self.forced = True
            return self.result
        except:
            raise SchemeError
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError # The number of inputted argument values does not match with the number of formal parameters
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN Question 5A
        "*** REPLACE THIS LINE ***"
        env.define(target, scheme_eval(expressions.second.first, env)) #an expression needed to be evaluated
        return target
        # END Question 5A
...
def check_formals(formals):
    ...
    while formals != nil:
        if not scheme_symbolp(formals.first) or formals.first in lst:
            raise SchemeError("Parameters invalid")
        lst += [formals.first]
        formals = formals.second
    # END Question 11B
